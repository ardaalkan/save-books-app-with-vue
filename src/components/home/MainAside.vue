<template>
  <aside
    class="bg-white flex flex-shrink-0 flex-col h-[100vh] w-44 border-r-[1px] border-gray-200 md:w-60"
  >
    <a
      v-for="category in categoryList"
      @click.prevent="$emit('category-changed', category.id)"
      :key="category.id"
      class="sidebar-item"
      href="#"
    >
    ...
    <a
      @click.prevent="$emit('category-changed', null)"
      class="sidebar-item"
      href="#"
    >
      <svg
...
axiosInstance.get("/categories").then((category_response) => {
